...
NSDictionary *parseTrustKitConfiguration(NSDictionary *TrustKitArguments)
{
    ...
    for (NSString *domainName in TrustKitArguments[kTSKPinnedDomains])
    {
        ...
        NSNumber *shouldIncludeSubdomains = @([domainName rangeOfString:WildcardDomainPrefix].location == 0);
        if ([shouldIncludeSubdomains boolValue] == YES)
        {
            ...
            NSString* domainNameOnly = [domainName substringFromIndex:WildcardDomainPrefix.length];
            if (GetRegistryLength([domainNameOnly UTF8String]) == [domainNameOnly length])
            {
                [NSException raise:@"TrustKit configuration invalid"
